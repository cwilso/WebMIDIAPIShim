...
(function (global, navigator) {
    'use strict';
    var midiIO, MIDIAccess, MIDIPort, MIDIInput, MIDIOutput, _requestMIDIAccess, _onReady, _midiProc, _onNotReady, _sendLater;
    ...
    function _JazzInstance() {
        ...
        if (this._Jazz) {
            ...
            this._Jazz._perfTimeZero = global.performance.now();
        }
    }
    ...
    MIDIAccess = function MIDIAccess() {
        ...
        if (this._jazzInstances[0]._Jazz) {
            ...
            global.setTimeout( _onReady.bind(this), 3 );
        } else {
            global.setTimeout( _onNotReady.bind(this), 3 );
        }
    };
    ...
    _onNotReady = function _onNotReady() {
        if (this._promise)
    ...
    _sendLater = function _sendLater() {
    ...
    MIDIOutput.prototype.send = function( data, timestamp ) {
        ...
        if (data.length === 0)
            return false;
        if (timestamp)
            delayBeforeSend = Math.floor( timestamp - global.performance.now() );
        if (timestamp && (delayBeforeSend>1)) {
            var sendObj = new Object();
            sendObj.jazz = this._jazzInstance;
            sendObj.data = data;
            global.setTimeout( _sendLater.bind(sendObj), delayBeforeSend );
        } else {
    ...
    if (!navigator.requestMIDIAccess)
        navigator.requestMIDIAccess = _requestMIDIAccess;
