...
function _midiProc( timestamp, status, data0, data1 ) {
  console.log( "MIDI data received (hex): " + status.toString(16) + " " + data0.toString(16) + " " + data1.toString(16) );

  var event = { type: "message" };
  event.timestamp = parseFloat( timestamp.toString());
  event.data = new Uint8Array(3);
  event.data[0] = status;
  event.data[1] = data0;
  event.data[2] = data1;
  if (this.onmessage)
...
MIDIOutput.prototype.send = function( data, timestamp ) {
  if (data.length==0)
    return false;
  if (data.length < 4)
    this._jazzInstance.MidiOut( data[0], (data.length>1) ? data[1] : null, (data.length>2) ? data[2] : null );
  else {
    // handle send as sysex
  }
  return true;
}
